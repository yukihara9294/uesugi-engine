...
class ODPTCollector:
    ...
    def _make_request(self, endpoint, params=None):
        ...
        try:
            ...
            logger.info(f"レスポンスデータ件数: {len(data) if isinstance(data, list) else 'N/A'}")
            
            # データの一部をサンプルとして出力（最初の1件）
            if isinstance(data, list) and len(data) > 0:
                logger.info(f"サンプルデータ（最初の1件）:")
    ...
    def collect_operators(self):
        ...
        if operators:
            ...
            # JR西日本や広島・山口エリアの事業者を探す
            relevant_operators = []
            for op in operators:
                op_id = op.get('owl:sameAs', '')
                op_name = op.get('dc:title', '')
                if any(keyword in op_name.lower() for keyword in ['西日本', 'jr-west', '広島', '山口', 'hiroshima', 'yamaguchi']):
                    relevant_operators.append(f"{op_id}: {op_name}")
            
            if relevant_operators:
                logger.info("広島・山口エリアに関連する可能性のある事業者:")
                for op in relevant_operators:
                    logger.info(f"  - {op}")
            else:
                logger.warning("広島・山口エリアに関連する事業者が見つかりませんでした")
    ...
    def run(self, regions=None):
        ...
        logger.info(f"APIキー: {'設定済み' if self.api_key else '未設定'}")
        ...
        if operators:
            logger.info("\n=== 利用可能な全事業者一覧 ===")
            for op in operators:
                logger.info(f"  - {op.get('owl:sameAs', 'ID不明')}: {op.get('dc:title', '名称不明')}")
