...
最終更新: 2025-06-14  
更新者: Claude Code  
更新内容: パーティクルフロー5秒停止問題修正、MDファイル統合整理
...
### 現在のフェーズ
- **仕様検討段階**: 広島県・山口県の取得データを可視化し、仕様を検討中
- **6地域展開の準備段階**: 広島・山口に続き、岡山・福岡・東京・大阪への展開準備

### 今後の展開
1. 広島・山口データの可視化完成
2. 岡山データの追加と6地域統一フォーマット設計
3. 福岡・東京・大阪データの統合
4. AI因果推論機能の実装
5. 全国展開
...
2. 冗長な説明禁止  
3. 重複実装禁止
4. バージョン派生を作らない（v2, simple等の命名禁止）

### デザイン方針
- **基本方針**: ダミーデータ時の表現を完全再現
- **パーティクル**: 2025年6月8日実装のサイバーパンク表現（ベジエ曲線、グロー効果、北側迂回）
- **SNSヒートマップ**: 青→赤のグラデーション  
- **宿泊施設**: 緑色（#4CAF50）、透明度50%、高さは収容人数に比例
- **ランドマーク**: 黄色（#FFD700）の円柱、全て3D表示
- **イベント**: オーブ状の光る表現、#FF6B6Bカラー
...
**原因**: ポート競合（Docker外でnpm startが既に実行されている）
**解決方法**:
1. `lsof -i :3000` でポート使用状況確認
2. 使用中の場合は `kill -9 <PID>` で停止
