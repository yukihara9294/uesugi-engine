...
class GTFSIntegrator:
    ...
    def load_gtfs_data(self, region="hiroshima"):
        ...
        # GTFSデータディレクトリ
        gtfs_dir = self.data_dir / region / "transport" / "bus" / "gtfs"
        
        if not gtfs_dir.exists():
            logger.error(f"GTFSディレクトリが見つかりません: {gtfs_dir}")
            return
            
        # 各事業者のGTFSファイルを処理
        for operator_dir in gtfs_dir.iterdir():
            if operator_dir.is_dir():
                logger.info(f"事業者データを処理中: {operator_dir.name}")
                self._process_operator_gtfs(operator_dir)
